...
//Precondition: hString and fp are handles to valid String and File objects
//Postcondition: Similar to string_extraction in that SUCCESS if it
// successfully wrote to the FILE* and FAILURE if it failed to do so,
// except that this function does not skip over leading blanks and will read
// all characters including spaces until it encounters a new line (or EOF).
// The function should consume the newline character from the input buffer
// but should not store it as one of the characters of the string.
